...
export default function Notificationglobalwl() {
     const reportSectionRef = useRef(null);
     const [records, setRecords] = useState([]);
     const [fullForm, setFullForm] = useState(false);

     const [searchCriteria, setSearchCriteria] = useState({
          Type: 'val4', // Assuming this determines the table to query
     ...
     const fetchRecords = () => {
          axios.post('http://localhost:56478/api/Rithual/notification', { type: "cal4" })
              .then(response => {
                  if (Array.isArray(response.data)) {
                      setRecords(response.data.map(record => ({ ...record, SendByEmail: false, SendByPost: false })));
                  } else {
     ...
     const handleValidateClick = (record) => {
          const updateInfo = {
               ...
               Type: "val4" // or "local" based on the context
     ...
     const submitUpdates = () => {
          ...
          axios.post('http://localhost:56478/api/Rithual/UpdateValidationsall', {
              Updates: updates,
              Type: "val4" // Adjust based on your current context or make it dynamic
          })
          .then(response => {
     ...
     const generatePDF = async (record) => {
          const requestData = {
              NumeroTitreipp: record.nummarque_int,
              NumeroTitrecon: record.nummarque_c,
              type: 'val4' // Assuming `type` is part of `record`
          };
