...
export default function RechercheTribunalup() {
  ...
  const handleSearch = async (event) => {
    ...
    const searchPayload = {
      ...
      DateJugDebut: formatDate(formData.get("RechercheTribunalDecisionStartDate")),
      DateJugFin: formatDate(formData.get("RechercheTribunalDecisionEndDate")),
      DateEnrngDebut: formatDate(formData.get("RechercheTribunalRegistrationStartDate")),
      DateEnrngFin: formatDate(formData.get("RechercheTribunalRegistrationEndDate")),
      CurrentPage: currentPage,
      PageSize: pageSize,
    };

    try {
      const response = await fetch("http://192.168.2.111:56478/api/Tribunal/searchup", {
        method: "POST",
        headers: {
  ...
  const handleSearchload = async (event) => {
    console.log("handleSearchload started");
    if (event) event.preventDefault();
    ...
    try {
      ...
      console.log("Date a week ago:", weekAgo);
      ...
      const searchPayload = {
        ...
        DateJugDebut: "",
        DateJugFin: "",
        DateEnrngDebut: formatDate(weekAgo),
        DateEnrngFin: formatDate(today),
        CurrentPage: currentPage,
        PageSize: pageSize,
      };
      ...
      setTotalRecords(uniqueResults.length);
  
    } catch (error) {
      console.error("Error in handleSearchload:", error);
      setSearchResults([]);
      setTotalRecords(0);
    } finally {
